    ...
    public class PortafolioDA
    {
        ...
        public static DataSet getPortafolio(string strFecha, int intCodCavali)
        {
            ...
            try
            {
                ...
                {
                    ds = getPortafolioORACLE(strFecha, intCodCavali);
                }
                else
                {
        ...
        public static DataSet getPortafolioORACLE(string strFecha, int intCodCavali)
        {
            try
            {
                OracleConnection conn = new OracleConnection(ConfigurationManager.ConnectionStrings["OracleConnection"].ConnectionString);
                conn.Open();
                OracleCommand Command = new OracleCommand("SELECT FLAG, CODIGO AS Valor, CANTIDAD_T AS Cantidad, CANTIDAD_D AS Disponible, " +
                                                    ...
                                                    "WHERE codcli=" + intCodCavali + " and TO_CHAR(FECCORTE, 'DD/MM/YYYY')='" + strFecha + "' AND (flag ='0' OR flag='1'))*100 AS ECTOT " +
                                                    "FROM TESTCARWEB WHERE " +
                                                    "CODCLI=" + intCodCavali + " and TO_CHAR(FECCORTE, 'DD/MM/YYYY')='" + strFecha + "' AND (FLAG=1 OR FLAG=0)" +
